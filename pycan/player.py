"""
Replays CAN traffic saved with pycan.logger back
to a CAN bus.

Similar to canplayer in the pycan-utils package.
"""
...
import pycan
from pycan import Bus, LogReader, MessageSync


def main():
    parser = argparse.ArgumentParser(
        "python -m pycan.player",
        description="Replay CAN traffic.")

    parser.add_argument("-f", "--file_name", dest="log_file",
                        help="""Path and base log filename, for supported types see pycan.LogReader.""",
                        default=None)
    ...
    parser.add_argument('infile', metavar='input-file', type=str,
                        help='The file to replay. For supported types see pycan.LogReader.')
    ...
    pycan.set_logging_level(logging_level_name)
