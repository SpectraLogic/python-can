...
from pycan.listener import BufferedReader
from pycan.message import Message
from .generic import BaseIOHandler

log = logging.getLogger('pycan.io.sqlite')
...
class SqliteWriter(BaseIOHandler, BufferedReader):
    ...
    thread. Ensures that all messages that are added before calling :meth:`~pycan.SqliteWriter.stop()`
    are actually written to the database after that call returns. Thus, calling
    :meth:`~pycan.SqliteWriter.stop()` may take a while.
    ...
    .. note::
        ...
        will continue receiving until the :attr:`~pycan.SqliteWriter.MAX_TIME_BETWEEN_WRITES`
        timeout is reached or more than
        :attr:`~pycan.SqliteWriter.MAX_BUFFER_SIZE_BEFORE_WRITES` messages are buffered.
