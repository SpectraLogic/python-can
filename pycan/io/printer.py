...
from pycan.listener import Listener
from .generic import BaseIOHandler

log = logging.getLogger('pycan.io.printer')


class Printer(BaseIOHandler, Listener):
    """
    The Printer class is a subclass of :class:`~pycan.Listener` which simply prints
    any messages it receives to the terminal (stdout). A message is turned into a
    string using :meth:`~pycan.Message.__str__`.
