...
TODO: We could implement this interface such that setting other filters
      ...
      after the connection was opened? We need to document that behavior!
...
from pycan import BusABC, Message
from pycan.broadcastmanager import (LimitedDurationCyclicSendTaskABC,
                                    RestartableCyclicTaskABC)
from pycan.ctypesutil import CLibrary, HANDLE, PHANDLE, HRESULT as ctypes_HRESULT
...
log = logging.getLogger('pycan.ixxat')
...
class IXXATBus(BusABC):
    ...
    .. warning::
        ...
        the filters have to be set in :meth:`~pycan.interfaces.ixxat.IXXATBus.__init__`
        using the ``can_filters`` parameter. Using :meth:`~pycan.interfaces.ixxat.IXXATBus.set_filters`
        does not work.
    ...
    def set_filters(self, can_filers=None):
        """Unsupported. See note on :class:`~pycan.interfaces.ixxat.IXXATBus`.
        """
