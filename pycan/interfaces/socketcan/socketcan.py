...
import pycan
from pycan import Message, BusABC
from pycan.broadcastmanager import ModifiableCyclicTaskABC, \
    RestartableCyclicTaskABC, LimitedDurationCyclicSendTaskABC
from pycan.interfaces.socketcan.constants import * # CAN_RAW, CAN_*_FLAG
...
if not HAS_NATIVE_SUPPORT:
    def check_status(result, function, arguments):
        if result < 0:
            raise pycan.CanError(error_code_to_str(ctypes.get_errno()))
        return result
...
def send_bcm(bcm_socket, data):
    ...
    try:
        ...
        if e.errno == errno.EINVAL:
            raise pycan.CanError(base + "You are probably referring to a non-existing frame.")

        elif e.errno == errno.ENETDOWN:
            raise pycan.CanError(base + "The CAN interface appears to be down.")

        elif e.errno == errno.EBADF:
            raise pycan.CanError(base + "The CAN socket appears to be closed.")
...
class CyclicSendTask(LimitedDurationCyclicSendTaskABC,
    ...
    def __init__(self, bcm_socket, message, period, duration=None):
        ...
        :param pycan.Message message: The message to be sent periodically.
    ...
    def modify_data(self, message):
        ...
        Note the Message must have the same :attr:`~pycan.Message.arbitration_id`
...
def capture_message(sock, get_channel=False):
    ...
    try:
        ...
        raise pycan.CanError("Error receiving: %s" % exc)
...
class SocketcanBus(BusABC):
    """
    Implements :meth:`pycan.BusABC._detect_available_configs`.
    """
    ...
    def __init__(self, channel="", receive_own_messages=False, fd=False, **kwargs):
        ...
        :param str channel:
            ...
            channel using :attr:`pycan.Message.channel`.
        :param bool receive_own_messages:
            If transmitted messages should also be received by this bus.
        :param bool fd:
            If CAN-FD frames should be supported.
        :param list can_filters:
            See :meth:`pycan.BusABC.set_filters`.
        """
    ...
    def _recv_internal(self, timeout):
        ...
        try:
            ...
            raise pycan.CanError("Failed to receive: %s" % exc)
    ...
    def send(self, msg, timeout=None):
        ...
        :param pycan.Message msg: A message object.
        :param float timeout:
        ...
        raise pycan.CanError("Transmit buffer full")
    ...
    def _send_once(self, data, channel=None):
        try:
            ...
            raise pycan.CanError("Failed to transmit: %s" % exc)
        return sent
    ...
    def _send_periodic_internal(self, msg, period, duration=None):
        ...
        :rtype: pycan.interfaces.socketcan.CyclicSendTask
