...
from pycan import CanError, Message, BusABC
from pycan.bus import BusState
from pycan.util import len2dlc, dlc2len
from .basic import *
...
# Set up logging
log = logging.getLogger('pycan.pcan')
...
class PcanBus(BusABC):
    ...
    def __init__(self, channel='PCAN_USBBUS1', state=BusState.ACTIVE, bitrate=500000, *args, **kwargs):
        ...
        On top of the usual :class:`~pycan.Bus` methods provided,
        the PCAN interface includes the :meth:`~pycan.interface.pcan.PcanBus.flash`
        and :meth:`~pycan.interface.pcan.PcanBus.status` methods.

        :param str channel:
            The can interface name. An example would be 'PCAN_USBBUS1'
            Default is 'PCAN_USBBUS1'

        :param pycan.bus.BusState state:
