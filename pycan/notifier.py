...
logger = logging.getLogger('pycan.Notifier')


class Notifier(object):

    def __init__(self, bus, listeners, timeout=1.0, loop=None):
        """Manages the distribution of :class:`pycan.Message` instances to listeners.
        ...
        :param pycan.BusABC bus: A :ref:`bus` or a list of buses to listen to.
        :param list listeners: An iterable of :class:`~pycan.Listener`
    ...
    def add_bus(self, bus):
        ...
        if self._loop is not None and hasattr(bus, 'fileno') and bus.fileno() >= 0:
            ...
            reader = threading.Thread(target=self._rx_thread, args=(bus,),
                                      name='pycan.notifier for bus "{}"'.format(bus.channel_info))
            reader.daemon = True
            reader.start()
        self._readers.append(reader)

    def stop(self, timeout=5):
        """Stop notifying Listeners when new :class:`~pycan.Message` objects arrive
        and call :meth:`~pycan.Listener.stop` on each Listener.
    ...
    def add_listener(self, listener):
        ...
        :param pycan.Listener listener: Listener to be added to
    ...
    def remove_listener(self, listener):
        ...
        :param pycan.Listener listener: Listener to be removed from
                         the list to be notified
