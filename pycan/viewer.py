...
logger = logging.getLogger('pycan.serial')
...
def parse_args(args):
    ...
    parser = argparse.ArgumentParser('python -m pycan.viewer',
    ...
    optional.add_argument('--version', action='version', help="Show program's version number and exit",
                          version='%(prog)s (version {version})'.format(version=pycan.__version__))
    ...
    optional.add_argument('-d', '--decode', dest='decode',
                          help='R|Specify how to convert the raw bytes into real values.'
                               ...
                               '\n  $ python -m pycan.viewer -d "100:<BHL"'
...
def main():  # pragma: no cover
    ...
    bus = pycan.Bus(parsed_args.channel, **config)
