...
try:
    # Only raise an exception on instantiation but allow module to be imported
...
class ThreadSafeBus(ObjectProxy):
    """
    Contains a thread safe :class:`pycan.BusABC` implementation that
    ...
    Use this as a drop-in replacement for :class:`~pycan.BusABC`.

    .. note::

        This approach assumes that both :meth:`~pycan.BusABC.send` and
        :meth:`~pycan.BusABC._recv_internal` of the underlying bus instance can be
        called simultaneously, and that the methods use :meth:`~pycan.BusABC._recv_internal`
        instead of :meth:`~pycan.BusABC.recv` directly.
    """
