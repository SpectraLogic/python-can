"""
This module contains the implementation of `pycan.Listener` and some readers.
"""
...
class BufferedReader(Listener):
    """
    A BufferedReader is a subclass of :class:`~pycan.Listener` which implements a
    **message buffer**: that is, when the :class:`pycan.BufferedReader` instance is
    ...
    :meth:`~pycan.BufferedReader.get_message`.

    Putting in messages after :meth:`~pycan.BufferedReader.stop` has be called will raise
    an exception, see :meth:`~pycan.BufferedReader.on_message_received`.
    ...
    def get_message(self, timeout=0.5):
        ...
        :meth:`pycan.BufferedReader.stop` has been called.

        :param float timeout: The number of seconds to wait for a new message.
        :rytpe: pycan.Message or None
