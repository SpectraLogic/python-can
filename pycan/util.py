...
import pycan
from pycan.interfaces import VALID_INTERFACES

log = logging.getLogger('pycan.util')
...
if platform.system() == "Linux":
    ...
    CONFIG_FILES.extend(
        [
            'pycan.ini',
            os.path.join(os.getenv('APPDATA', ''), 'pycan.ini')
        ]
    )
...
def load_config(path=None, config=None, context=None):
    ...
    Interface can be any of the strings from ``pycan.VALID_INTERFACES`` for example:
    ...
    pycan.log.debug("can config: {}".format(config))
    return config
...
def set_logging_level(level_name=None):
    ...
    """
    can_logger = logging.getLogger('pycan')
