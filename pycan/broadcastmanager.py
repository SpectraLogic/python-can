...
log = logging.getLogger('pycan.bcm')
...
class ModifiableCyclicTaskABC(CyclicSendTaskABC):
    ...
    def modify_data(self, message):
        ...
        :param pycan.Message message:
          The message with the new :attr:`pycan.Message.data`.
...
def send_periodic(bus, message, period, *args, **kwargs):
    """
    Send a :class:`~pycan.Message` every `period` seconds on the given bus.

    :param pycan.BusABC bus: A CAN bus which supports sending.
    :param pycan.Message message: Message to send periodically.
    :param float period: The minimum time between sending messages.
    :return: A started task instance
    """
    warnings.warn("The function `pycan.send_periodic` is deprecated and will " +
                  "be removed in an upcoming version. Please use `pycan.Bus.send_periodic` instead.", DeprecationWarning)
