...
class ZeroDLCTest(unittest.TestCase):
    ...
    def test_recv_non_zero_dlc(self):
        bus_send = pycan.interface.Bus(bustype='virtual')
        bus_recv = pycan.interface.Bus(bustype='virtual')
        data = [0, 1, 2, 3, 4, 5, 6, 7]
        msg_send = pycan.Message(is_extended_id=False, arbitration_id=0x100, data=data)
    ...
    def test_recv_zero_dlc(self):
        bus_send = pycan.interface.Bus(bustype='virtual')
        bus_recv = pycan.interface.Bus(bustype='virtual')
        msg_send = pycan.Message(is_extended_id=False, arbitration_id=0x100, data=[])
